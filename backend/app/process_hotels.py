...
def filter_and_rank_hotels(df, user_preferences):
    ...
    for idx, row in df_filtered.iterrows():
        ...
        for amenity in amenities:
            if amenity not in mentioned_amenities and amenity in row and row[amenity] == True:
                weight = 0.3
                max_score += weight
    ...
    return df_filtered.sort_values(by="match_score", ascending=False).reset_index(drop=True)
