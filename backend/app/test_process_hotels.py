...
import pandas as pd
import json
from process_hotels import normalize_value, filter_and_rank_hotels, find_matching_hotels
...
class TestProcessHotels(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_hotels = pd.DataFrame({
            ...
            'roomcategory': ['Deluxe Double Room', 'Standard Double Room', 'Economy Double Room']
        })
    ...
    def test_normalize_value(self):
        ...
        self.assertTrue(normalize_value('distancetocity', 1) > normalize_value('distancetocity', 5))

        # Test price normalization (inverse relationship)
        self.assertTrue(normalize_value('pricepernight', 100) > normalize_value('pricepernight', 200))
